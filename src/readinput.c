...
void ReadInputFile(int argc, char *argv[]) {
    ...
    while (fgets(line, MAXCHAR, FileInputPtr) != NULL) {
        ...
        if (strcasecmp("NumberOfEquilibrationCycles", keyword) == 0)
            sscanf(arguments, "%d", &NumberOfEquilibrationCycles);
        ...
        if (strcasecmp("TranslationProbability", keyword) == 0)
            sscanf(arguments, "%lf", &ProbabilityOfDisplacementMove);
    ...
    while (fgets(line, MAXCHAR, AdsorbateInput) != NULL) {
        ...
        if (strcasecmp("NumberOfAdsorbateAtom", keyword) == 0) sscanf(arguments, "%d", &NumberOfAdsorbateAtom);
            ...
            //printf("Keyword: %-30s\t Argument: %-20s\n", keyword, arguments);
            sscanf(arguments, "%s %lf %lf %lf %lf %lf", Symbol[aa], &Epsilon[aa], &Sigma[aa], &Mass[aa], &Charge[aa],
                   &AdsorbateAtomicDistance[aa]);
...
void SetSimulationUnits(void) {
    ...
    Lambda3_Sigma3 = CUBE(
            sqrt(SQR(PLANCK_CONSTANT) / (2 * M_PI * MassInSI * BOLTZMANN_CONSTANT * TempInSI * SQR(SigmaInSI))));
