  ...
  for(i=1;i<=2;i++)
  {
    ...
    for(j=1; j < NumberOfCycles + 1; j++)
    {
      ...
      for(k=0;k<NumberOfSteps;k++)
      {
        ...
        else if (rand > ProbabilityOfExchangeMove && rand <= ProbabilityOfExchangeMove + ProbabilityOfDisplacementMove)
    ...
    if(NumberOfCycles != 0)
    {
      if (NumberOfDisplacementAttempts != 0)
      {
        ...
        fprintf(FileOutput,"Success: %d (%.2lf %%)\n", NumberOfAcceptedDisplacementMoves, 100.0 * NumberOfAcceptedDisplacementMoves / NumberOfDisplacementAttempts);
      }

      if (NumberOfRotationAttempts != 0)
        {
            fprintf(FileOutput, "Number Of Rotation move attempts  : %d\n", NumberOfRotationAttempts);
            fprintf(FileOutput,"Success: %d (%.2lf %%)\n", NumberOfAcceptedRotationMoves, 100.0 * NumberOfAcceptedRotationMoves / NumberOfRotationAttempts);
        }
      if (NumberOfExchangeAttempts !=0)
      {
          fprintf(FileOutput,"Number Of Exchange move attempts  : %d \n", NumberOfExchangeAttempts);
          fprintf(FileOutput,"Addition Success : %d (%.2lf %%) \n", NumberOfAcceptedAdditionMoves, 100.0* NumberOfAcceptedAdditionMoves/NumberOfExchangeAttempts);
          fprintf(FileOutput,"Deletion Success : %d (%.2lf %%) \n", NumberOfAcceptedDeletionMoves, 100.0* NumberOfAcceptedDeletionMoves/NumberOfExchangeAttempts);
      }
