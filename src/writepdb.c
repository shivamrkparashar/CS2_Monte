...
void WritePdb(FILE *FilePtr) {
    ...
    fprintf(FilePtr, "CRYST1%9.3lf%9.3lf%9.3lf%7.2lf%7.2lf%7.2lf\n", Sigma[primaryatom] * Box, Sigma[primaryatom] * Box,
            Sigma[primaryatom] * Box, 90.0, 90.0, 90.0);

    for (i = 0; i < NumberOfParticles; i++) {
        CountMolecule++;
        for (int aa = 0; aa < NumberOfAdsorbateAtom; aa++)
            fprintf(FilePtr, "ATOM%7d  %2s%12d   %8.3lf%8.3lf%8.3lf\n",
                    (i + 1), Symbol[aa], aa, Sigma[primaryatom] * Positions[i][aa].x,
                    Sigma[primaryatom] * Positions[i][aa].y, Sigma[primaryatom] * Positions[i][aa].z);
    }
    fprintf(FilePtr, "%s\n", "ENDMDL");
}
