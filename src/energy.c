        ...
        for (int aa = 0; aa < NumberOfAdsorbateAtom; aa++)
        {
            for (int aa2 = 0; aa2< NumberOfAdsorbateAtom; aa2++)
            {
                ...
                if (r2 < SQR(VdWCutOff)) {
                    ...
                    // Shifted potential
                    UvdWff += 4.0 * epsilon_factor * (SQR(sigma_factor_6 * r6i) - sigma_factor_6 * r6i) - PotentialatCutoff;
                ...
                if (r2 < SQR(ChargeCutOff)) {
                    ...
                    URealEwaldff += COULOMB_CONVERSION_FACTOR * q1 * q2 * ErrorFunctionComplement(Alpha * rij) / rij;
                }
...
double CylindricalPotential(VECTOR *PosPtr)
{
    ...
    for (int aa=0; aa<NumberOfAdsorbateAtom; aa++)
    {
        ...
        phi3_term = M_PI/2.0*pow(ss/(R + alpha*Delta)/(1-SQR(r/(R + alpha*Delta))), 3)*hypergeometric(-1.5, -0.5, 1, SQR(r/(R + alpha*Delta)));
